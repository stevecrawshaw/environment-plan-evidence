...
```{r libraries}
pacman::p_load(tidyverse, janitor, readxl, glue, fs, duckdb, arrow, DBI, gt, gtExtras, treemapify, openair, emojifont, ggtext, patchwork, showtext, fs, ragg)
```

```{r global-variables}

# get named vectors of colours from WECA brand guidelines and make color vectors
...
weca_secondary_colours <- jsonlite::read_json(
  "https://raw.githubusercontent.com/westofengland-ca/weca_templates/refs/heads/main/General_branding/brand_guidelines.json"
) |>
...
# Make a universal theme for the bar plots
...
# better graphics device - important for text rendering in the pie plot with fontawesome
...
# vector of 4 colours for use later
weca_cols_4 <- weca_core_colours[2:5] |> 
  unname() |> 
...
emissions_treemap_plot <- emissions_data_weca_max_year_tbl |>
  ...
  scale_fill_manual(values = weca_cols_4) +
    labs(
...
# --- STEP 1: Use a simple, non-synced local path ---
# Ensure you have copied the font file to C:/temp/fonts/
font_add(
  family = "fontawesome", 
  regular = "C:/Temp/fonts/fontawesome-webfont.ttf"
)
...
fuel_types_homes_plot_tbl <- epc_domestic_lep_tbl |> 
  ...
  mutate(category = if_else(str_starts(category, "Renewable"), "Renewable inc. heat pumps", category),
    ...
    # colors for use in later plots
weca_cols_6 <- weca_secondary_colours[1:6] |> 
  unname() |> 
...
fuel_types_homes_plot_tbl |>
  ...
  geom_col(show.legend = FALSE, fill = weca_cols_4[1]) +
  coord_flip() +
...
fuel_types_homes_single_bar <-
  ...
  coord_flip() +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_manual(values = weca_cols_6) +
...
# show a GT table with the numbers and percentages of homes by heating fuel type
...
# make a stacked col plot of emissions by local authority and 
...
save_tables <- function(gt_table, name){
...
all_gt_objects |> 
  imap(~ save_tables(.x, .y)) |> invisible()
