...
energy_data_weca_max_year_all_fuels_tbl <- energy_data_all_la_tbl |>
  ...
  group_by(sector) |>
  summarise(energy_GTOe = sum(value, na.rm = TRUE), .groups = "drop") |>
...
fa_font_path <- "www/fontawesome-webfont.ttf"
...
emissions_weca_plot <- emissions_weca_plot_tbl |>
  ...
  labs(
    ...
    subtitle = glue("West of England local authorities, {start_year}-{max_year}"),
    x = "Year",
    y = "KtCO2e", ## AMENDED: Corrected the call to quickText() which is not a standard function.
    fill = "Local authority",
    caption = "Source: UK local authority and regional greenhouse gas emissions statistics"
  ) +
  coord_flip() +
...
# Use imap to iterate through the list of plots and save each one
...
## AMENDED: Renamed function for consistency and used walk2 for side-effects.
# The original code had a stray invisible() call which is not needed with imap/walk.
